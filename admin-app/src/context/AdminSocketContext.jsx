...
export function AdminSocketProvider({ children }) {
  ...
  const [bugCountInfo, setBugCountInfo] = useState(null); // { requestedBugCount, actualBugCount }
  ...
  useEffect(() => {
    ...
    const onStarted = ({ room, requestedBugCount, actualBugCount }) => {
      setRoom(room);
      setBugCountInfo({ requestedBugCount, actualBugCount });
    };
  ...
  const value = { room, participants, leaderboard, joinError, bugCountInfo, joinRoom, startGame, endGame };
