...
export function registerPlayerHandlers(io, socket) {
  socket.on('player:join', ({ roomCode, name }) => {
    ...
    if (!room) {
      return socket.emit('player:joinError', {
    ...
    // maxParticipants=0 significa que a sala está fechada para novas entradas.
    if (
    ...
    io.to(`admin:${room.code}`).emit('room:participantJoined', {
      participant,
    });
  });
  ...
  socket.on('player:submitCode', ({ code }) => {
    ...
    const isCorrect =
      normalize(code) === normalize(room.expectedFix);
    ...
    if (isCorrect) {
      ...
      const elapsedSeconds =
        (Date.now() - (participant.startedAt || Date.now())) / 1000;

      const timeLeftSeconds = Math.max(
        0,
        room.timeLimit - elapsedSeconds
      );

      const { bonus, totalGained } = applyCorrectAnswer(
        participant,
        timeLeftSeconds,
        room.timeLimit
      );
  ...
  // Quando um participante desconecta:
  // - se ainda estava esperando, é removido da sala;
  // - se estava resolvendo, é marcado como failed_time para não
  //   deixar a partida travada indefinidamente.
  socket.on('disconnect', () => {
    ...
    if (participant.status === 'solving') {
      participant.status = 'failed_time';

      updateRoom(room.code, room);

      io.to(`admin:${room.code}`).emit(
        'game:participantUpdate',
        {
          participantId,
          status: participant.status,
          score: participant.score,
...
function checkGameEnd(io, room) {
  ...
  io.to(`admin:${room.code}`).emit('game:finished', {
    leaderboard,
  });
