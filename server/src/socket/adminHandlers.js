...
export function registerAdminHandlers(io, socket) {
  ...
  socket.on('admin:startGame', ({ roomCode }) => {
    ...
    // Gera UM código com N erros para todos os participantes.
    // bugsApplied.length é a quantidade REAL de erros presentes no código.
    // O injectBugs garante que não há sobreposição entre eles.
    const { buggyCode, expectedFix, bugsApplied } = injectBugs(
      room.sourceCode,
      room.bugCount || 3
    );

    const actualBugCount = bugsApplied.length;

    room.buggyCode = buggyCode;
    room.expectedFix = expectedFix;
    room.bugsApplied = bugsApplied;
    room.actualBugCount = actualBugCount;
    room.status = 'running';
    ...
    room.participants.forEach((participant) => {
      ...
      io.to(participant.socketId).emit('game:code', {
        ...
        bugCount: actualBugCount,
      });
    });

    updateRoom(room.code, room);

    io.to(`admin:${room.code}`).emit('game:started', {
      room: sanitizeRoom(room),
      requestedBugCount: room.bugCount || 3,
      actualBugCount,
    });
  });
  ...
  socket.on('admin:kickParticipant', ({ roomCode, participantId }) => {
    ...
    io.to(`admin:${room.code}`).emit('room:participantLeft', {
      participantId,
    });
  });
}
